...
var vm = new _vue2.default({
    ...
    ready: function ready() {
        ...
        //解决浏览器记住密码的功能
        setTimeout(function () {
            _this.spypasswd = 'password';
            _this.spyconfirmPassword = 'password';
        }, 200);
        //一次性的标识，防止用户去登录后回到首页也弹出自己的优惠券
        if (_vue2.default.sessionStorage.getItem('adRegis')) {
            this.adRegis_flag = true;
            _vue2.default.sessionStorage.removeItem('adRegis');
        }
    ...
    methods: {
        ...
        nextStep: function nextStep() {
            ...
            //提交注册
            var url = _config2.default.ouserHost + '/ouser-web/api/user/checkMobileCaptcha.do';
            var param = {
            ...
            _vue2.default.api.postForm(url, param, function (res) {
                ...
                if (res.code) {
                    ...
                    if (res.code == "0") {
                        ...
                        msg = "注册成功";
                        //标识是首页广告点击过来注册的用户
                        if (_this2.adRegis_flag) {
                            _vue2.default.sessionStorage.setItem('adRegisBack', 1);
                        }
                        if (res.isPwd) {
                            //这个判断没有意义
                            if (!_this2.fromUrl) {
        ...
        getRegisText: function getRegisText() {
            var _this3 = this;
            ...
            _vue2.default.api.get(url, param, function (res) {
                _this3.reg_protocol = res.data.reg_protocol[0].linkUrl || '';
            }, function () {
                //处理掉不显示报错
            });
        },
        //发送验证码
        checkViCode: function checkViCode() {
            var _this4 = this;
            ...
            //校验手机号是否已注册
            var url = _config2.default.ouserHost + '/ouser-web/api/user/checkAccountRepeat.do';
            var param = {
                companyId: _vue2.default.mallSettings.getCompanyId(),
                mobile: this.user.mobile
            };
            _vue2.default.api.postForm(url, param, function (res) {
                ...
                setTimeout(function () {
                    location.href = _config2.default.contextPath + "/login.html";
                }, 3000);
            });
        },
        //发送验证码
        smsSend: function smsSend() {
            ...
            var inter = setInterval(function () {
                _this5.waitingTime--;
                _this5.smsBtn = _this5.waitingTime + 's';
                if (_this5.waitingTime === 0) {
                    _this5.smsBtn = '重新获取';
                    _this5.smsBtnDisabled = false;
                    _this5.waitingTime = 60;
                    clearInterval(inter);
                }
            }, 1000);
            ...
            _vue2.default.api.postForm(url, param, function (res) {
                //验证码发送成功
                _this5.startImgCode = false; //隐藏图片验证码窗口
            }, function (res) {
                if (res.code == 913) {
        ...
        setPassword: function setPassword() {
            ...
            var url = _config2.default.ouserHost + '/ouser-web/api/user/register.do';
            var param = {
                mobile: this.user.mobile,
                password: this.user.password,
                inviteMobile: this.user.inviteMobile,
        ...
        login: function login() {
            ...
            _vue2.default.api.postForm(url, param, function (res) {
                _this7.sbmDisabled = false;
                _vue2.default.auth.setUserToken(res.ut);
                if (!_this7.fromUrl) {
                    location.href = _config2.default.contextPath + "/index.html";
                } else {
                    var toUrl = _this7.fromUrl;
                    window.location.href = toUrl;
                }
            }, function (res) {
                _this7.sbmDisabled = false;
        ...
        // 超限发送验证码
        overLimitSmsSend: function overLimitSmsSend() {
            var _this8 = this;
            ...
            _vue2.default.api.postForm(url, param, function (res) {
                //验证码发送成功
                _this8.startImgCode = false; //隐藏图片验证码窗口
                _this8.smsBtnDisabled = true;
                _this8.smsBtn = _this8.waitingTime + 's';
                var inter = setInterval(function () {
                    _this8.waitingTime--;
                    _this8.smsBtn = _this8.waitingTime + 's';
                    if (_this8.waitingTime === 0) {
                        _this8.smsBtn = '重新获取';
                        _this8.smsBtnDisabled = false;
                        _this8.waitingTime = 60;
                        clearInterval(inter);
                    }
                }, 1000);
                _vue2.default.utils.showTips(res.message);
        ...
        // 验证邀请人手机号是否注册
        judgeRegis: function judgeRegis(params) {
            var _this9 = this;

            if (!this.user.password || !/^[^\u2E80-\u9FFF]{6,18}$/.test(this.user.password)) {
                _vue2.default.utils.showTips('密码6-16个字符');
                return;
            }
            ...
            if (!this.inviteFlag) {
                if (this.user.inviteMobile && !/^[0-9]{11}$/.test(this.user.inviteMobile)) {
                    _vue2.default.utils.showTips('邀请人手机号格式错误');
                    return;
                }
                ...
                _vue2.default.api.postForm(url, param, function (res) {
                    _vue2.default.utils.showTips('推荐人不存在');
                    return;
                }, function (res) {
                    _this9.setPassword();
                });
