...
new _vue2.default({
    ...
    methods: {
        ...
        //     })
        // },
        getAskObj: function getAskObj(flag) {
            var _this2 = this;

            var url = _config2.default.apiHost + '/back-product-web/consultAppAction/getQaDetailList.do';
            var params = {
            ...
            _vue2.default.api.post(url, params, function (res) {
                if (res.data) {
                    _this2.askObj = res.data;
                    if (flag) {
                        _this2.answerList = res.data.listObj || [];
                    } else {
                        _this2.answerList = _this2.answerList.concat(res.data.listObj || []);
                    }
                    if (_this2.pageSize > res.data.listObj.length) {
                        _this2.isEnd = true;
                    }
                    _this2.getRightLimit();
                }
            });
        },
        answerQuestion: function answerQuestion(params) {
            var _this3 = this;

            if (!this.content) {
                _vue2.default.utils.showTips('请输入回复内容');
                return;
            }
            if (!this.limitRight) {
                _vue2.default.utils.showTips('暂无权限回答该问题哦~');
                return;
            }
            var url = _config2.default.apiHost + '/back-product-web/consultAppAction/answerTheConsult.do';
            var params = {
                consultItemId: urlParams.askId,
            ...
            _vue2.default.api.post(url, params, function (res) {
                _this3.pageNo = 1;
                _this3.getAskObj(true);
                _vue2.default.utils.showTips('回复成功');
                _this3.content = '';
                _this3.getRightLimit();
            });
        },
        getRightLimit: function getRightLimit(params) {
            var _this4 = this;

            var url = _config2.default.apiHost + '/back-product-web/consultAppAction/validatePublish.do';
            var params = {
