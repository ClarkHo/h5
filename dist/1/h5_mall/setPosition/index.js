...
new _vue2.default({
    ...
    methods: {
        ...
        getSearch: function getSearch(keyWord, flag) {
            var _this2 = this;
            ...
            autocomplete.search(keyWord, function (status, result) {
                ...
                if (flag) {
                    _this2.stepThreeSearchList = tempArray;
                } else {
                    _this2.stepOneSearchList = tempArray;
                }
            });
        },
        // 根据经纬度获取地址
        getAddressByInglat: function getAddressByInglat(lnglatXY) {
            var _this3 = this;

            geocoder.getAddress(lnglatXY, function (status, result) {
                if (status === 'complete' && result.info === 'OK' && result.regeocode.formattedAddress) {
                    _this3.getSearch(result.regeocode.formattedAddress, true);
                }
            });
        },
        //返回
        stepOneBack: function stepOneBack() {
            if (this.stepOne == 1) {
                history.back();
            } else {
                this.stepOne = 1;
        ...
        selectCity: function selectCity(item) {
            ...
            this.cityList.forEach(function (v) {
        ...
        getCity: function getCity() {
            ...
            var url = _config2.default.apiHost + '/rest/search/merchant/serviceAreaList.do';
            _vue2.default.api.get(url, null, function (res) {
                ...
                if (res.data && res.data.length > 0) {
                    ...
                    _this4.cityList = res.data || [];
                    _this4.choosedPos = _this4.cityList[0].areaName;
                    _this4.initGeocoder();
                }
            });
        },
        ...
        //使用上面的工具
        useGeocoder: function useGeocoder() {
            var _this7 = this;

            if (!this.stepThreeKeyWord) return;
            geocoder.getLocation(this.stepThreeKeyWord, function (status, result) {
                if (status === 'complete' && result.info === 'OK') {
                    //TODO:获得了有效经纬度，可以做一些展示工作
                    //比如在获得的经纬度上打上一个Marker
                    //尝试在地图上定位
                    try {
                        _this7.setLocation(result.geocodes[0].location);
                    } catch (error) {}
                } else {
                    //获取经纬度失败
                }
