...
var vm = new _vue2.default({
    ...
    ready: function ready() {
        ...
        if (this.isFromPay && urlParams.addr) {
            _vue2.default.geo.getLocationByAddress(decodeURI(urlParams.addr), function (loc) {
                _this.loc = loc;
                _this.loadShopList();
            });
        } else {
            ...
            if (_vue2.default.browser.isApp()) {
                _vue2.default.app.postMessage("getLocation", function (loc) {
                    if (typeof loc == "string") {
                        _this.loc = JSON.parse(loc);
                    } else {
                        _this.loc = loc;
                    }
                    _this.loadShopList();
        ...
        _vue2.default.scrollLoading(function () {
            if (_this.shopList.length < _this.totalCount) {
                _this.pageNo += 1;
                _this.loadShopList();
            }
        });
    ...
    methods: {
        ...
        getLoc: function getLoc() {
            ...
            map.plugin('AMap.Geolocation', function () {
                ...
                AMap.event.addListener(geolocation, 'error', function () {
                    // 获取经纬度出错，使用默认地址
                    _vue2.default.utils.showTips('定位失败');
                    that.loc = {
        ...
        //失去焦点
        inputOnBlur: function inputOnBlur(obj) {
            setTimeout(function () {
                obj.flag = false;
            }, 50);
        },
        ...
        loadShopList: function loadShopList() {
            ...
            //如果没有给定地址,使用默认地址
            if (!loc) {
                loc = _vue2.default.geo.getDefaultLocation();
            }
            var params = { platformId: _config2.default.platformId, keyword: this.keyword.value, longitude: loc.longitude, latitude: loc.latitude,
            ...
            _vue2.default.api.get("/api/read/merchant/queryBaseMerchantList", params, function (result) {
                if (result.data) {
                    var data = result.data;
                    _this2.totalCount = data.totalNum || 0;
                    if (data.dataList && data.dataList.length > 0) {
                        _this2.shopList = _this2.shopList.concat(data.dataList);
                        //this.clearMarkers();
                        _this2.addMarkers();
                    }
                }
                _this2.setLocation(loc);
            });
        },
        ...
        //打开门店详情
        openShopDetail: function openShopDetail(merchantId, index) {
            var _this3 = this;

            this.selectMarker(index);
            var params = { platformId: _config2.default.platformId, merchantId: merchantId };
            _vue2.default.api.get("/api/read/merchant/getBaseMerchantShopInfo", params, function (result) {
                if (result.data) {
                    _this3.currentShopInfo = result.data;
                    _this3.telNum = _this3.currentShopInfo.phone;
                    _this3.businessTime = _this3.currentShopInfo.businessTime;
                    _this3.latitude = _this3.currentShopInfo.latitude;
                    _this3.longitude = _this3.currentShopInfo.longitude;
                    _this3.location = _this3.currentShopInfo.location;
        ...
        //选择门店
        selectShop: function selectShop(m_id) {
            ...
            for (var i = 0; i < this.shopList.length; i++) {
                ...
                if (shop.longitude && shop.latitude) {
                    ...
                    AMap.event.addListener(marker, 'click', function (e) {
                        ...
                        //显示详情
                        _this4.openShopDetail(extData.merchantId, extData.index);
                    });
                }
            }
        },
        ...
        //get marker content
        getMarkerContent: function getMarkerContent(index, selected) {
            if (selected) {
        ...
        navigate: function navigate() {
            _vue2.default.app.postMessage("navigate", { longitude: this.longitude, latitude: this.latitude, location: this.currentShopInfo.address });
