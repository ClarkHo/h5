...
new _vue2.default({
    ...
    ready: function ready() {
        ...
        this.orderCode = _vue2.default.utils.paramsFormat(window.location.href).orderCode;
        this.loadOrderDetail();
        _vue2.default.getSysTime(function (res) {
            _this2.sysTime = res.data.timestamp || 0;
        });
    },
    ...
    methods: {
        ...
        loadOrderDetail: function loadOrderDetail() {
            ...
            _vue2.default.api.get(_config2.default.apiHost + "/api/my/order/detail", params, function (result) {
                _this3.order = result.data;
                _this3.getCancleTime(_this3.order.cancelTime);
            });
        },
        ...
        confirmReceived: function confirmReceived(order) {
            ...
            dialog.on("dialog:action", function (e) {
                //点击确定按钮
                if (e.index == 1) {
                    var params = { ut: _this4.ut, orderCode: order.orderCode };
        ...
        cancelOrder: function cancelOrder() {
            ...
            dialog.on("dialog:action", function (e) {
                //点击确定按钮
                if (e.index == 1) {
                    var params = { ut: _this5.ut, orderCode: _this5.orderCode };
                    _vue2.default.api.postForm(_config2.default.apiHost + "/api/my/order/cancel", params, function (result) {
                        //更改订单为已取消状态
                        _this5.order.orderStatus = 10;
                    });
                }
            });
        },
        //针对拼团订单的去支付链接
        goPayWay: function goPayWay(orderCode, instId) {
            instId = instId ? '_' + instId : '';
            if (_vue2.default.browser.weixin()) {
                location.href = _vue2.default.utils.getPayUrl(orderCode + instId);
            } else if (_vue2.default.browser.isApp()) {
                var url = 'yh://pay?body={"orderCode":"' + orderCode + '","amount":' + this.order.paymentAmount + ', "url": "' + _vue2.default.utils.getPayBackUrl(orderCode + '_' + instId) + '"}';
                location.href = url;
            } else {
