...
new _vue2.default({
    ...
    ready: function ready(params) {
        ...
        _vue2.default.scrollLoading({
            ...
            callback: function callback() {
                ...
                if (!_this.subCategoryCache[_this.currentCategoryId].isEnd) {
                    _this.scrollLoading();
                    _this.pageNo += 1;
                }
            }
    ...
    watch: {
        ...
        childrenCategories: {
            handler: function handler() {
                try {
                    var amount = 0;
                    for (var v in this.subCategoryCache) {
                        this.subCategoryCache[v].dataArray.forEach(function (item) {
                            if (item.num > 0) {
    ...
    methods: {
        getSearchList: function getSearchList(params) {},
        //获取一级分类
        getParentCategories: function getParentCategories() {
            ...
            _vue2.default.api.get("/api/category/list", params, function (result) {
                if (result.data && result.data.categorys) {
                    _this2.parentCategories = result.data.categorys;
                    _this2.initScrollYL();
                    //默认显示推荐
                    if (_this2.parentCategories.length > 0) {
                        if (!_this2.currentCategoryId) {
                            //this.getRecommend(); 默认推荐
                            _this2.currentCategoryId = _this2.parentCategories[0].categoryId;
                            _this2.getChildrenCategories(_this2.currentCategoryId);
                        } else {
                            _this2.getChildrenCategories(_this2.currentCategoryId);
                        }
                    }
        ...
        getChildrenCategories: function getChildrenCategories(parentId, flag) {
            var _this3 = this;
            ...
            if (this.subCategoryCache[parentId]) {
                ...
                setTimeout(function () {
                    _this3.childrenCategories = _this3.subCategoryCache[parentId].dataArray;
                }, 100);
            ...
            _vue2.default.api.get(_config2.default.apiHost + '/api/search/searchList', params, function (result) {

                if (result.data && result.data.productList) {
                    var mpIds = [];
                    result.data.productList.forEach(function (item) {
                        item.num = 0;
                        mpIds.push(item.mpId);
                    });
                    if (result.data.productList.length == 0) {
                        _this3.childrenCategories = [];
                        _this3.subCategoryCache[parentId] = {
                            "pageNo": 1,
                            "dataArray": _this3.childrenCategories,
                            "isEnd": false
                        };
                    }
                    _vue2.default.getPriceAndStock(mpIds.join(), result.data.productList, null, function (obj) {
                        _this3.childrenCategories = obj;
                        //cache
                        _this3.subCategoryCache[parentId] = {
                            "pageNo": 1,
                            "dataArray": _this3.childrenCategories,
        ...
        scrollLoading: function scrollLoading() {
            ...
            _vue2.default.api.get(_config2.default.apiHost + '/api/search/searchList', params, function (result) {
                ...
                if (result.data && result.data.productList) {
                    ...
                    _vue2.default.getPriceAndStock(mpIds.join(), result.data.productList, null, function (obj) {
                        _this4.childrenCategories = _this4.childrenCategories.concat(obj || []);
                        //cache
                        var flag = result.data.productList.length < _this4.pageSize ? true : false;
                        _this4.subCategoryCache[_this4.currentCategoryId] = {
                            "pageNo": _this4.pageNo,
                            "dataArray": _this4.childrenCategories,
                            "isEnd": flag
                        };
                        // Vue.set(this.subCategoryCache[parentId],'pageNo',1);
                        // Vue.set(this.subCategoryCache[parentId],'dataArray',this.childrenCategories);
        ...
        checkNum: function checkNum(item) {
            if (item.num < 0) {
                _vue2.default.utils.showTips('购买数量不能小于1');
                item.num = 1;
                return;
            }
            if (item.num > item.stockNum) {
                _vue2.default.utils.showTips('库存不足');
        ...
        addToCart: function addToCart(params) {
            ...
            this.getTempSkus(function (skus) {
                var params = {
        ...
        //获取选中的商品skus
        getTempSkus: function getTempSkus(cb) {
            var tempSkus = [];
            for (var v in this.subCategoryCache) {
                this.subCategoryCache[v].dataArray.forEach(function (item) {
                    if (item.num > 0) {
