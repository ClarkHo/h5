...
vm = new Vue({
    el: 'body',
    components: {UiScrollTop,UiActionsheetPop,UiHeader,UiDropDown,UiSerialProduct},
    data: {
        placehoder:'',
        rightNavFlag: false,
    ...
    ready: function() {
        ...
        if (Vue.pageRestorer.restore()) {
            ...
            if(Vue.auth.loggedIn() && this.showSearchPop){
                this.pageNoView = 1;
        ...
        Vue.scrollLoading({triggerHeight: 1000, callback: () => {
            if(!this.showSearchPop && !this.showSequencePop && this.totalCount>0){
                ...
                if(this.footList.length > 0){
                    //看过的商品
                    if (this.pageNoView < Math.ceil(this.totalCountView/this.pageSizeView)) {
                        this.pageNoView += 1;
                        this.getFootList();
                    }else {
                        this.showNoMore = true;
    ...
    methods: {
        ...
        getCartCount:function(){
            "use strict";
            var params = {sessionId: this.sessionId, ut: Vue.auth.getUserToken()};
        ...
        clickAutoFill: function (str) {
            ...
            str = str.replace(/[ | ]*\n/g,'\n'); //去除行尾空白
            window.location.href = config.contextPath + "/search.html?keyword=" + str;
        },
        ...
        getSerialProducts: function (item) {
            Vue.api.get("/back-product-web/api/product/serialProducts.do", {mpId: item.mpId,platformId:config.platformId}, (result) => {
                if (result.data) {
                    this.serialAttributes = result.data.attributes || [];
                    this.serialProducts = result.data.serialProducts || [];
                    if(this.serialProducts.length > 0){// 产品属性 > 0的话显示弹出框 否则直接添加购物车 功能
        ...
        getItemInfo: function (itemId) {
            Vue.api.get("/api/product/baseInfo", {mpsIds: itemId}, (result) => {
                if (result.data && result.data.length > 0) {
                    ...
                    if(this.itemInfo.isSeries == 1){
                        this.showSizePop=true;
                    } else{
                        this.addItemInCart(this.itemInfo);
                    }
                }
            });
        },
        ...
        submitAllSelection:function() {
            ...
            //跳转
            var url = "/search.html?filter=1" +
                (this.prices.from.length>0||this.prices.to.length>0?"&originalPrice=" + this.prices.from + ',' + this.prices.to :'')+
        ...
        getFootList: function () {
            ...
            Vue.api.get("/api/my/foot/list", params, (res) => {
                var temp = [],mpIds = [];
                ...
                Vue.getPriceAndStock(mpIds.join(),temp,null,(obj)=>{
                    this.footList = this.footList.concat(obj);
                });
                this.resizeImgHeight();
                this.totalCountView = res.data.totalCount;
            });
        },
        //重定向
        gotoCart:function(){
